...
test('static hosting can refresh official results into shared rooms', () => {
  ...
  assert.match(html, /function mapFifaCalendarMatchesToKoFixtures\(matches=\[\]\)/);
  ...
  assert.match(html, /payload\.state\.koFixtures=mergedFixtures;/);
...
test('static hosting uses shared Firestore rooms instead of per-device local rooms', () => {
  ...
  assert.match(html, /ss\(roomDataKey\(normalized\.room\.id,'koFixtures'\),normalized\.state\.koFixtures\|\|\{\}\);/);
  assert.match(html, /ss\(roomDataKey\(roomId,'koFixtures'\),roomState\.koFixtures\|\|\{\}\);/);
  ...
  assert.doesNotMatch(html, /onManualSync=\{syncCurrentRoom\}/);
  assert.doesNotMatch(html, /最新状態に更新/);
...
test('knockout score row uses score hit wording', () => {
  ...
  assert.match(html, /\{label:'PK勝者的中',me:koPenaltyHitPts\(myKoSummary\),opp:koPenaltyHitPts\(oppKoSummary\)\}/);
