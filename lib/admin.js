const DEFAULT_ADMIN_TOKEN = 'WC2026';

function verifyAdminToken(inputToken, serverToken = DEFAULT_ADMIN_TOKEN) {
  const expected = String(serverToken ?? DEFAULT_ADMIN_TOKEN).trim();
  if (!expected) {
    return {
      ok: false,
      status: 503,
