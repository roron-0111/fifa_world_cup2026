...
function getCanonicalOfficialTeamName(team) {
  const name = getOfficialTeamName(team);
  return name ? canonicalTeamName(name) : null;
}
...
function fixtureSnapshotSignature(fixtures = {}) {
  return JSON.stringify(
    Object.keys(fixtures || {})
      .sort()
      .map((matchId) => [matchId, fixtures[matchId]]),
  );
}
...
function mapFifaCalendarMatchesToKoFixtures(matches = []) {
  const fixtures = {};
  for (const match of matches || []) {
    if (!match || typeof match !== 'object') continue;
    const matchNo = toFiniteNumber(match.MatchNumber ?? match.matchNumber ?? match.matchNo);
    const matchId = matchIdFromMatchNumber(matchNo);
    if (!matchId) continue;
    const home = getCanonicalOfficialTeamName(match.Home ?? match.home);
    const away = getCanonicalOfficialTeamName(match.Away ?? match.away);
    if (!home && !away) continue;
    const fixture = { matchNo };
    if (home) fixture.home = home;
    if (away) fixture.away = away;
...
async function fetchFifaWorldCupResults({ fetchImpl = globalThis.fetch } = {}) {
  ...
  const koFixtures = mapFifaCalendarMatchesToKoFixtures(matches);
  return {
    results,
    koFixtures,
